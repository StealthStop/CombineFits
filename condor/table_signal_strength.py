...
def makePValuePlot(path, dataSet, approved, suffix, close):
    # CL observed pvalues
    ...
    if approved:
        c1.Print(path + "/output-files/" + dataSet["runtype"]+"_"+dataSet["model"]+"_"+suffix+dataSet["pdfName"]+".pdf")
    else:
        c1.Print(path + "/output-files/" + dataSet["runtype"]+"_"+dataSet["model"]+"_"+suffix+close+dataSet["pdfName"]+"_prelim.pdf")
    del c1

def makePValuePlotOverlap(dataSets, approved, close, pdfname):
    # CL observed pvalues
    pvalue_0l = dataSets["0l"]["pList"]
    pvalue_1l = dataSets["1l"]["pList"]
    pvalue_combo = dataSets["combo"]["pList"]
    print pvalue_0l
    print pvalue_1l
    print pvalue_combo
    xpoints = dataSets["0l"]["mList"]
    npoints = len(pvalue_0l)
    Xmin = 300
    Xmax = 1200
    Ymin = 5.0e-37
    Ymax = 1
    ...
    gr_0l = ROOT.TGraph(npoints, array('d', xpoints), array('d', pvalue_0l))
    gr_0l.SetLineColor(ROOT.kRed+1)
    gr_0l.SetLineStyle(4)
    gr_0l.SetLineWidth(3)

    gr_1l = ROOT.TGraph(npoints, array('d', xpoints), array('d', pvalue_1l))
    gr_1l.SetLineColor(ROOT.kBlue+1)
    gr_1l.SetLineStyle(3)
    gr_1l.SetLineWidth(3)

    gr_combo = ROOT.TGraph(npoints, array('d', xpoints), array('d', pvalue_combo))
    gr_combo.SetLineColor(ROOT.kGreen+1)
    gr_combo.SetLineStyle(2)
    gr_combo.SetLineWidth(3)
    ...
    #elif dataSet["model"]=="StealthSYY": legend1.SetHeader("pp #rightarrow #tilde{t} #bar{#tilde{t}}, #tilde{t} #rightarrow t#tilde{S}g, #tilde{S} #rightarrow S#tilde{G}, S #rightarrow gg");
    #elif dataSet["model"]=="StealthSHH": legend1.SetHeader("pp #rightarrow #tilde{t} #bar{#tilde{t}}, SHH coupling");
    legend1.AddEntry(gr_0l, "0 Lepton", "l")
    legend1.AddEntry(gr_1l, "1 Lepton", "l")
    legend1.AddEntry(gr_combo, "Combo", "l")
    legend1.SetBorderSize(0)
    legend1.SetFillStyle(0)
    legend1.Draw("same")
    ...
    cmstext.SetTextFont(42)
    cmstext.SetTextAlign(31)
    cmstext.DrawLatex(1 - ROOT.gPad.GetRightMargin(), 1 - (ROOT.gPad.GetTopMargin() - 0.017), "137.2 fb^{-1} (13 TeV)")
    ...
    rvalue_Combo = array('d', dataSets["combo"]["rList"])
    rpvalue_Combo = array('d',dataSets["combo"]["rpList"])
    rmvalue_Combo = array('d', dataSets["combo"]["rmList"])
    zero = array('d', dataSets["combo"]["zero"])
    rband = ROOT.TGraphAsymmErrors(npoints, array('d', xpoints), rvalue_Combo, zero, zero, rmvalue_Combo, rpvalue_Combo)
    rband.SetFillColor(ROOT.kGreen+1)
    rband.Draw("3 same")
    r = ROOT.TGraph(npoints, array('d', xpoints), rvalue_Combo)
    ...
    if approved:
        c1.Print("pseudoDataS_RPV_overlap"+close+pdfname+".pdf")
    else:
        c1.Print("pseudoDataS_RPV_overlap"+close+pdfname+"_prelim.pdf")
    del c1
...
def main():
    ...
    overlap = {'0l': None, "1l": None, "combo": None}
    for dataSet in allData:
        pdfname = dataSet["dataset"]["pdfName"]
        if dataSet["dataset"]["model"] == "RPV":
            overlap[dataSet["dataset"]["suffix"]] = dataSet["dataset"]["data"]["2016"]
        ...
        for s in suffixes:
            if s == dataSet["suffix"]:
                makePValuePlot(path, dataSet["dataset"], options.approved, s, close)

    #print(overlap)
    makePValuePlotOverlap(overlap, options.approved, close, pdfname)
