...
def main():
    ...
    for st in signalType:
        ...
        for mass in masssets:
            ...
            if not options.toy and not options.toyS:
                outputFiles = [
                    "higgsCombine%s.AsymptoticLimits.mH%s.MODEL%s.root" % (options.year, mass, model),
                    "higgsCombine%s%s%s.FitDiagnomodelics.mH%s.MODEL%s.root" % (options.year, model, mass, mass, model),
                    "higgsCombine%s%s%s_SignifExp.Significance.mH%s.MODEL%s.root" % (options.year, model, mass, mass, model),
                    ...
                    "MVA_%s_%s_%s_ws.root"   % (options.year, model, mass),
                    "MVA_2016_%s_%s_ws.root" % (model, mass),
                    "MVA_2017_%s_%s_ws.root" % (model, mass),
                    "MVA_2018pre_%s_%s_ws.root" % (model, mass),
                    "MVA_2018post_%s_%s_ws.root" % (model, mass),
                    "ws_%s_%s_%s.root"       % (options.year, model, mass),
                    "fitDiagnomodelics%s%s%s.root" % (options.year, model, mass), 
                    "impacts_%s%s%s.json"       % (options.year, model, mass),
                    "impacts_%s%s%s_%s.pdf"     % (options.year, model, mass, options.dataType),
                    "log_%s%s%s_Asymp.txt"      % (options.year, model, mass),
                    "log_%s%s%s_FitDiag.txt"    % (options.year, model, mass),
                    "log_%s%s%s_Sign_sig.txt"   % (options.year, model, mass),
                    "log_%s%s%s_Sign_noSig.txt" % (options.year, model, mass),
                    "log_%s%s%s_multiDim.txt"   % (options.year, model, mass),
                    "log_%s%s%s_HybridNew.txt"  % (options.year, model, mass),
                    "dataCard%s%s%s.txt"        % (options.year, model, mass),
                ]
                ...
                fileParts.append("Arguments = %s %s %s %s %s %s %s %s %s %i %i %i %i %s %s\n" % (options.inputRoot2016, options.inputRoot2017, options.inputRoot2018pre, options.inputRoot2018post, 
                                                                                              model, stauxi2, mass, options.year, options.dataType, doAsym, doFitDiag, doMulti, doImpact, 
                ...
                for x in range(0, nSteps+1):                               
                    ...
                    for y in range(jPerR):                        
                        ...
                        fileParts.append(transfer)
                        fileParts.append("Arguments = %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s\n" % (options.inputRoot2016, options.inputRoot2017, options.inputRoot2018pre, options.inputRoot2018post,
    ...
    filestoTransfer = [environ["CMSSW_BASE"] + "/src/CombineFits/DataCardProducer/produceDataCard.py",
                       environ["CMSSW_BASE"] + "/src/CombineFits/DataCardProducer/dataCardProducer.py",
                       environ["CMSSW_BASE"] + "/src/CombineFits/DataCardProducer/cardConfigGoodClose.py",
                       ]
