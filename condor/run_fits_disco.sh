...
then
    ...
    combine -M AsymptoticLimits ${fitOptions} --verbose 2 --rMax $rMax                        -n ${year}${signalType}${mass}${dataType}${suffix}${close1}_AsymLimit > log_${year}${signalType}${mass}${dataType}${suffix}${close1}_Asymp.txt
    combine -M Significance     ${fitOptions} --verbose 2 --rMax $rMax -t -1 --expectSignal=1 -n ${year}${signalType}${mass}${dataType}${suffix}${close1}_SignifExp > log_${year}${signalType}${mass}${dataType}${suffix}${close1}_Sign_sig.txt
    combine -M Significance     ${fitOptions} --verbose 2 --rMax $rMax                        -n ${year}${signalType}${mass}${dataType}${suffix}${close1}_SignifExp > log_${year}${signalType}${mass}${dataType}${suffix}${close1}_Sign_noSig.txt
fi
...
then
    ...
    if [ $inject == 0 ] 
    then
        combine -M FitDiagnostics ${fitOptions} --verbose 2 --rMax $rMax --robustFit=1 --plots --saveShapes --saveNormalizations --saveWithUncertainties -n ${year}${signalType}${mass}${dataType}${suffix}${close1} > log_${year}${signalType}${mass}${dataType}${suffix}${close1}_FitDiag.txt
    else
        combine -M FitDiagnostics ${fitOptions} --verbose 2 --rMax $rMax --robustFit=1 --plots --saveShapes --saveNormalizations --saveWithUncertainties -n ${year}${signalType}${mass}${dataType}${suffix}${close1} -t -1 --toysFrequentist --expectSignal=${inject} > log_${year}${signalType}${mass}${dataType}${suffix}${close1}_FitDiag.txt
    fi
fi
