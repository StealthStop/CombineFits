...
for d in nuis_list:
        ...
        if nuis_x == None: row += [ " n/a " ]
            ...
            if nuis_p != None:
                if options.plotfile: 
                    ...
                    if len(nameParts) > 2:
                        badName = nameParts[2]; process = nameParts[1]; year = nameParts[-1]
                        if "d_" != newName[0:2]:
                            if process == "tt": newName = "t#bar{t}"

                            if badName in niceNames: newName += " %s"%(niceNames[badName])
                            else:                    newName += " %s"%(badName)

                            if "20" in year: newName += " (%s)"%(year.replace("pre", "A").replace("post", "B"))
    ...
    cmstext.DrawLatex(1 - ROOT.gPad.GetRightMargin() + 0.08, 1 - (ROOT.gPad.GetTopMargin() + 0.015), year)

    np1 = ROOT.TPaveText(0.66,0.72,0.79,0.82, "NDC")
    ...
    np1.Draw("SAME")
    np2 = ROOT.TPaveText(0.66,0.65,0.79,0.75, "NDC")
    ...
    mark.SetTextFont(61);
    mark.DrawLatex(-0.04 + ROOT.gPad.GetLeftMargin(), 1 - (ROOT.gPad.GetTopMargin() + 0.02), "CMS")
    mark.SetTextFont(52);
    if data:
        mark.DrawLatex(-0.04 + ROOT.gPad.GetLeftMargin() + 0.03, 1 - (ROOT.gPad.GetTopMargin() - 0.02), "")
    else:
    ...
    leg=ROOT.TLegend(0.81,0.66,0.90,0.85)
    leg.SetFillColor(0)
    leg.SetTextFont(42)
    ...
    dummyChi2.GetYaxis().SetTitleSize(0.10*PadFactor)
    dummyChi2.GetYaxis().SetTitleOffset(0.30/PadFactor)
    dummyChi2.GetYaxis().SetLabelSize(0.07*PadFactor)
    dummyChi2.GetXaxis().SetLabelSize(0.075*PadFactor)
    ...
    leg2=ROOT.TLegend(0.81,0.81,0.9,1.0)
    leg2.SetFillColor(0)
    leg2.SetTextFont(42)
